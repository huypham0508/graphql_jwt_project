...
import { ConfigJWT, Role } from "../../config";
...
class AuthMiddleware {
    ...
    async ({ context, info }, next) => {
      try {
        ...
        if (parentTypes.includes(parentTypeName)) {
          ...
          const parentName = this.getParentClass(
            info.fieldName,
            parentTypeName
          );
  ...
  private static getParentClass(
    ...
    switch (parentTypeName) {
      case "Query":
        const queries: ResolverMetadata | undefined = metadata.queries.find(
          (m) => m.methodName === fieldName
        );
        return queries?.target.name;
      case "Mutation":
        const mutations: ResolverMetadata | undefined = metadata.mutations.find(
          (m) => m.methodName === fieldName
        );
        return mutations?.target.name;
      case "Subscription":
        const subscriptions: ResolverMetadata | undefined =
          metadata.subscriptions.find((m) => m.methodName === fieldName);
...
const VerifyTokenForgotPassword = AuthMiddleware.verifyToken(
